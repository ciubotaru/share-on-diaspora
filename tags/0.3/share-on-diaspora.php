...
$bc = ( get_option('my-settings')['button_color'] != '' ) ? get_option('my-settings')['button_color'] : get_default('button_color');
// border-color: #aabbd2; border-width: 1px; color: #3c72c2;
//$bb = preg_match('/^[a-f0-9]{6}$/i', $_GET['bb']) ? $_GET['bb'] : 'ecf2f6';
$bb = ( get_option('my-settings')['button_background'] != '' ) ? get_option('my-settings')['button_background'] : get_default('button_background');
// background-color: #ecf2f6; 

switch (get_option('my-settings')['button_size']) {
...
$br =  ( get_option('my-settings')['button_rounded'] != '' ) ? get_option('my-settings')['button_rounded'] : get_default('button_rounded');
...
function my_admin_init() {
    register_setting( 'share_on_diaspora_options-group', 'my-settings', 'my_settings_validate' );
//    register_setting( 'share_on_diaspora_options-group', 'my-settings' );
    ...
    add_settings_field( 'button_background', 'Background color', 'my_text_input', 'share_on_diaspora_options', 'section-one', array(
        'name' => 'my-settings[button_background]',
        'value' => ((get_option( 'my-settings' )['button_background'] != '' ) ? get_option( 'my-settings' )['button_background'] : get_default('button_background')),
    ...
    add_settings_field( 'button_color', 'Text and border color', 'my_text_input', 'share_on_diaspora_options', 'section-one', array(
        'name' => 'my-settings[button_color]',
        'value' => ((get_option( 'my-settings' )['button_color'] != '' ) ? get_option( 'my-settings' )['button_color'] : get_default('button_color')),
        'comment' => 'A six-digit hexadecimal number like <code>000000</code> or <code>ffffff</code>. Leave empty to restore the default value.'
        )
    );
    add_settings_field( 'button_size', 'Button size', 'my_radio_group', 'share_on_diaspora_options', 'section-one', array(
        'name' => 'my-settings[button_size]',
        'value' => ((get_option( 'my-settings' )['button_size'] != '' ) ? get_option( 'my-settings' )['button_size'] : get_default('button_size')),
    ...
    add_settings_field( 'button_rounded', 'Rounded corners', 'my_radio_group', 'share_on_diaspora_options', 'section-one', array(
        'name' => 'my-settings[button_rounded]',
        'value' => ((get_option( 'my-settings' )['button_rounded'] != '' ) ? get_option( 'my-settings' )['button_rounded'] : get_default('button_rounded')),
...
//    $output = get_option( 'my-settings' );
    ...
    foreach ($colors as $i)
        ...
        if ($output[$i] != '')
            ...
            if ($output[$i] == '')
                {
                add_settings_error( 'my-settings', 'invalid-color', 'Invalid value for \''. $i . '\'. Reverting to default.' );
                }
            }
        else
            {
            add_settings_error( 'my-settings', 'missing-color', 'Value missing for \''. $i . '\'. Reverting to default.' );
            }
        }
    if ($output['reset'] != '')
        {
        add_settings_error( 'my-settings', 'reverted to defaults', 'All parameters reverted to their default values.' );
        return $defaults;
        }
    else return $output;
    }
...
function share_on_diaspora_options_page() {
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            ...
            <?php submit_button('Defaults', 'secondary', 'my-settings[reset]', false); ?>
        </form>
    </div>
    <?php
